...
class CandidateController extends Controller
{
                ...
                'rules' => [
                    [
                    	...
                    	'matchCallback' => function ($rule, $action) {
                    		return Yii::$app->user->identity->usertype == UsersBase::UserTypeCandidate;    // return boolean
                    	}
                    ],
                ],
            ],
    ...
    public function actionDashboard()
    {
    	...
    	{
    		$docApproved[$doc['web']] = array('app' => true , 'name' => $doc['name'] , 'web' => $doc['web']);
    		$docwebpathlist[] = $doc['web'];
    		$docappcount ++;
    	}
    	
    	$filemodels = UploadedfilesBase::findAll(['file' => $docwebpathlist]);
    	...
    	$photo_approved = true; 
    	$filemodel = UploadedfilesBase::findOne(['file' => $photopath]);
    	if($filemodel != null)
    	{
    	...
    	$skillModel = CandidateskillBase::findAll(['candidateid' => $candidateModel->userid]);
    	...
    	switch ($action)
    	{
    		case 'myprofile' : 	
    			...
    			$skills = SkillsBase::find()->where(['parentid' => 1 , 'status' => 1 , 'jobtype' => $candidateModel->jobtype])->orderBy('title')->all();
    			 
    			$candidSkills =  CandidateskillBase::findAll(['candidateid' => $candidateModel->userid]);
    			...
    			//print $candidateModel->desiredjobtimetype. '<hr>';
    			//print_r($worktypes_array); exit;
    		...
    		case 'deletemarkedjob':
    			...
    			if(isset($_GET['id']))
    			{
    				CandidatefavoriteBase::deleteAll(['userid' => Yii::$app->user->identity->id, 'jobposid' => $_GET['id']]);
    			}
    			
    			return $this->redirect(['candidate/dashboard/markedjob']);
    			break;
    		...
    		case 'deleteapplyjob':
    			...
    			if(isset($_GET['id']))
    			{
    					CandidatejobapplyBase::deleteAll(['userid' => Yii::$app->user->identity->id, 'jobposid' => $_GET['id']]);
    			}
    				 
    			return $this->redirect(['candidate/dashboard/applyjob']);
    			break;
    	...
    	$percentCalc += $candidateModel->address != null && $candidateModel->address != '' ? 1 : 0;
    	...
    	$percentCalc += $candidateModel->desiredjobpcode != null && $candidateModel->desiredjobpcode != '' ? 1 : 0;
    	$percentCalc += $candidateModel->desiredjobcity != null && $candidateModel->desiredjobcity != '' ? 1 : 0;
    	$percentCalc += $candidateModel->desiredjobcountry != null && $candidateModel->desiredjobcountry != '' ? 1 : 0;
    	$percentCalc += $candidateModel->desiredjobregion != null && $candidateModel->desiredjobregion != '' ? 1 : 0;
    	$percentCalc += $candidateModel->desiredjobtimetype != null && $candidateModel->desiredjobtimetype != '' ? 1 : 0;
    ...
    private function update_profile($part , $data)
    {
    	...
    	$model = UsersBase::findOne(['id' => Yii::$app->user->identity->id]); //Yii::$app->user->identity;
    	$candidateModel = CandidateBase::findOne(['userid' => $model->id]);
        ...
        if($part == "candidate_photo")
    	{
    		if(count($_FILES) > 0 && isset($_FILES['candidate_photo']))
    		{
	    		...
	    		$upload_path = $upload_path . '/' . Yii::$app->user->identity->id . '.' . $info['extension'];
	    		if(file_exists($upload_path))
	    		{
	    			unlink($upload_path);
	    		}
	    		...
	    		$photo_web_path = Yii::getAlias('@web') . '/web/candidate_photo/' . Yii::$app->user->identity->id . '.' . $info['extension'];
    	...
    	if($part == "contact")
    	{
    		...
    		{
    			echo Yii::t('app', 'Fehler : Üngultiges E-Mail!') ; exit;
    		}
    		
    		$data['UsersBase']['uname'] = $data['CandidateBase']['email'];
    		
    		$data['CandidateBase']['cellphone'] = $data['ctel1'] . '-' . $data['ctel2'] . '-' . $data['ctel3'];
    	...
    	if($part == "cur-job-situation")
    	{
    		...
    		$skills = SkillsBase::find()->where(['parentid' => 1 , 'status' => 1 , 'jobtype' => $data['CandidateBase']['jobtype']])->all();
    	...
    	if($part == "job-vision")
    	{
    		...
    		$data['CandidateBase']['desiredjobcountry'] = isset($data['CandidateBase']['desiredjobcountry']) ? $data['CandidateBase']['desiredjobcountry'] : false;
    		$data['CandidateBase']['desiredjobcity'] = isset($data['CandidateBase']['desiredjobcity']) ? $data['CandidateBase']['desiredjobcity'] : false;
    		$data['CandidateBase']['desiredjobpcode'] = isset($data['CandidateBase']['desiredjobpcode']) ? $data['CandidateBase']['desiredjobpcode'] : false;
    	...
    	if($part == "candidate_doc")
    	{
    		...
    		if(count($_FILES) > 0 && isset($_FILES['candidate_doc']))
    		{
    			...
    			$doc_web_path = Yii::getAlias('@web') . '/web/candidate_doc/' . Yii::$app->user->identity->id . '/' . $name;
    			$filemodel = UploadedfilesBase::findOne(['file' => $doc_web_path]);
    			if($filemodel != null)
    			{
    				$filemodel->upload_date = date('Y-m-d H:i:s');
    			}
   ...
   /**
    ...
    public function actionVerify()
    {
    	...
    	$candidateModel = CandidateBase::find()->where(['userid' => $userid])->one();
    ...
    public function actionMyprofile()
    {
    	...
    	if(count($_POST) && isset($_POST['checkcondition']))
    	{
    		...
    		$_POST['CandidateBase']['cellphone'] = $_POST['ctel1'] . $_POST['ctel2'] . $_POST['ctel3'];
    	...
    	$nationalities_array = array_merge($nationalities_array , BrainHelper::mapTranslate($nationalities, 'title', 'title'));
    	$countries_array = array('' => '');
    	$countries_array = array_merge($countries_array , BrainHelper::mapTranslate($countries, 'title', 'title'));
    	$distances_array = array('' => '');
    	$distances_array = array_merge($distances_array , BrainHelper::mapTranslate($distances, 'title', 'title'));
    	$titles_array = array('' => '');
    ...
    public function actionSkills($jt)
    {
    	...
    	$skills = SkillsBase::find()->where(['parentid' => 0 , 'status' => 1 , 'jobtype' => $jt])->one();
    	$parentid = 0;
		if(!$skills)
    	{
    		echo 'alert("Error!2")';
    	...
    	$skills = SkillsBase::find()->where(['parentid' => $parentid , 'status' => 1 , 'jobtype' => $jt])->orderBy('title')->all();
    	$skill_array = BrainHelper::mapTranslate($skills, 'id', 'title');
    	...
    	switch ($mode)
    	{
    		case 'email_exists':
    			...
    			if($checkemail)
    			{
    				...
    				$query = CandidateBase::find()->select(['userid' , 'email'])->where(['like' , 'email' , $checkemail])->andWhere(['=' , 'userid' , $currentid]);
    				...
    				{
    				    $query = CandidateBase::find()->select(['userid' , 'email'])->where(['like' , 'email' , $checkemail]);
    ...
    public function actionJobfav($id)
    {
    	...
    	$favModel = CandidatefavoriteBase::findOne(['userid' => Yii::$app->user->identity->id, 'jobposid' => $id]);
    	...
    	else 
    	{
    		...
    		$data = array('CandidatefavoriteBase' => array());
    	...
    	return $this->redirect(['site/jobview' , 'id' => $id]);
    }
    ...
    {
    	//return Yii::$app->user->identity->id . ' , ' . $id;
    	$favModel = CandidatejobapplyBase::findOne(['userid' => Yii::$app->user->identity->id, 'jobposid' => $id]);
    ...
    public function actionApplyjoblistjson()
    {
    	...
    	$list = [];
    	foreach ($jobadvList as $jobadv){
    		$jobtitle = strlen($jobadv->getJobpos()->title) > 42 ? substr($jobadv->getJobpos()->title , 0 , 40) . ' ...' : $jobadv->getJobpos()->title;
    		$item = [];
    		$item['id'] = $jobadv->getJobpos()->id;
    		$item['jobtitle'] = $jobtitle;
       		$item['status'] = $jobadv->getJobpos()->status;
    		$item['jobstartdate'] = $formatter->asDate($jobadv->getJobpos()->jobstartdate, 'php:d.m.Y');
    ...
    public static function personal_documents()
    {
    	...
    	foreach ($list as $docfile)
    	{
    		...
    		$files[] = array('web' =>  Yii::getAlias('@web') . '/web/candidate_doc/' . Yii::$app->user->identity->id . '/' . $info['basename'] , 'name' => $info['basename'] , 'path' => $docfile);
    	}
