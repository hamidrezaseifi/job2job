...
<div class="register-bewerbung" ng-controller="CandidateContentController">
    ...
    ?></div>
		    	<div class="item">
		    	<?=BrainRadioBoxRenderer::widget(['name' => 'CandidateBase[title]','value' => $candidateModel->title,'id_prefix' => 'title','items' => BrainStaticList::titleList(false)]);?>
				</div>
		    	<div class="item-title"><?php
    ...
    ActiveForm::end()?>
			...
			<div class="items-edit-preview-part">
				<div class="applyedit" title="<?php

    echo Yii::t('app', 'übernehmen');
    ?>">&nbsp;</div>
				<div class="showedit" title="<?php

    echo Yii::t('app', 'Bearbeiten');
    ?>">&nbsp;</div>
				<div class="hideedit" title="<?php
    ...
    echo Yii::t('app', 'Postleitzahl,Stadt');
    ?></div>
		    	<div class="item">
    ...
    echo Yii::t('app', 'Straße,Hausnummer');
    ?></div>
		    	<div class="item">
		    		<input type="text" name="CandidateBase[address]" value="<?=$candidateModel->street?>" />
		    	</div>
		    	<div class="item-title">&nbsp;</div>
		    	<div class="item">&nbsp;</div>
		    	<div class="item-title"><?php

    echo Yii::t('app', 'Mobiltelefon');
    ?></div>
		    	<div class="item">
    ...
    echo $model->attributeLabels()['receive_backend_email'];
    ?></div>
    ...
    echo Yii::t('app', 'Wann dürfen wir Sie kontaktieren?');
    ?></div>
		    	<div class="item">
    ...
    echo Yii::t('app', 'Straße,Hausnummer');
    ?></div>
		    	<div class="item"><?=($candidateModel->street)?></div>
		    	<div class="item-title">&nbsp;</div>
		    	<div class="item">&nbsp;</div>
		    	<div class="item-title"><?php
    ...
    echo Yii::t('app', 'AKTUELLE BERUFLICHE SITUATION');
    ?>
    	</div>
 	    <div class="register-bewerbung-teil-items-container">
    ...
    foreach ($skills as $skill) {
        ?>
					<span class="skillitempreview"><?php
...
echo Yii::t('app', 'BERUFLICHE VORSTELLUNG');
?>
			<div class="anlagen-notiz"><?php

echo Yii::t('app', 'Wo möchten Sie Ihre Tätigkeit ausführen?');
?> </div>
	    </div>
	 	<div class="register-bewerbung-teil-items-container">
...
echo Yii::t('app', 'ANSCHREIBEN & ANLAGEN');
?>
			<div class="anlagen-notiz"><?php

echo Yii::t('app',
    'Sie haben die möglichkeit Anschreiben, Lebensläufe, Zeugnisse und andere Anlagen anzuhängen. Die einzelnen Dateien dürfen 8 MB nicht überschreiten. Insgesamt düren maximal 20 MB hochgelagen werden. Erlaubte Dateiformate sind .pdf und .doc(.docx , .rtf).');
?> </div>
	    </div>
	 	<div class="register-bewerbung-teil-items-container">
    ...
    echo Yii::t('app', 'Anschreiben');
    ?></div>
		    	<div class="item" style="height: inherit;"><?=nl2br($candidateModel->coverletter)?></div>

		    	<div class="item-title" style=";"><?php

    echo Yii::t('app', 'Anlagen hinzufügen');
    ?></div>
		    	<div class="item"><?php
    ...
    if (count($docFiles) == 0) {
        ...
        foreach ($docFiles as $doc) {
            ...
            echo (! $doc['app'] ? 'text-decoration: line-through;' : '')?>" target="_blank" ><?php

            echo $doc['name'];
            ?></a><?php

            echo (! $doc['app'] ? Yii::t('app', ' (nicht bestätigt!)') : '');
            ?><br>
		    			<?php
        }
    }
    ...
    ?></div>
		    	...
		    	<div class="item-title" style="margin-top:115px;"><?php
...
<div id="skillbrowser" ng-show="showskillbrower" class="fade-in-out"  >
	<div class="skillbrowspannel">
		<div style="width: 100%;">
			<div class="skillbrowstitle"><?php

echo Yii::t('app', 'Fähigkeit hinzufügen');
?></div>
		...
		<div class="skillbrowsbottom">
			<button id="addskill" ng-click="addskill()"><?php

echo Yii::t('app', 'Hinzufügen');
?></button>
		</div>
...
?>;
	var skills_title = <?php

echo json_encode(array_values($allskills));
?>;
	var skills_id = <?php

echo json_encode(array_keys($allskills));
?>;

	var anrede_msg = "<?php

echo Yii::t('app', 'Bitte wählen Sie eine Anrede aus!');
?>";
	var fname_msg = "<?php

echo Yii::t('app', 'Bitte geben Sie ihre Vorname an!');
?>";
	var lname_msg = "<?php

echo Yii::t('app', 'Bitte geben Sie ihre Nachname an!');
?>";
	var bdate_msg = "<?php

echo Yii::t('app', 'Bitte geben Sie ihr Geburtsdatum an!');
?>";
	var email_msg = "<?php

echo Yii::t('app', 'Bitte geben Sie ihr E-Mail an!');
?>";
	var email_invalid_msg = "<?php

echo Yii::t('app', 'Das E-Mail ist ungültig.\nBitte geben Sie ihr E-Mail an!');
?>";
	var email_exists_msg = "<?php

echo Yii::t('app', 'Das E-Mail existiert in unserem Datenbank.\nBitte geben Sie anderes E-Mail an!');
?>";
	var curstatus_msg = "<?php

echo Yii::t('app', 'Bitte erklären Sie ihre Beschäftigung!');
?>";
	var avail_msg = "<?php

echo Yii::t('app', 'Bitte erklären Sie ihre Verfügberkeit!');
?>";
	var condition_msg = "<?php
...
echo Yii::getAlias('@web/candidate/');
?>";
	var skillurl = "<?php

echo Yii::getAlias('@web/candidate/skills');
?>";
