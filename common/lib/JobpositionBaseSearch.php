...
class JobpositionBaseSearch extends JobpositionBase
{
	...
	{
		return [
				[['id', 'companyid', 'status'], 'integer'],
				[['title', 'subtitle', 'postcode', 'city', 'country', 'createdate', 'updatedate'], 'safe'],
		];
	}
	...
	public function search($params, $order = array('createdate' => SORT_DESC), $filterExpire = true, $fromJob2job = false)
	{
		...
		if($filterExpire)
		{
			$query->andWhere(['>=', 'expiredate', date('Y-m-d')]);
		}
		
		if($fromJob2job)
		{
			$job2jobModels = CompanyBase::find()->where(['isjob2job' => 1, 'status' => 1])->select(['id'])->all();
			
			$allCompID = ArrayHelper::getColumn($job2jobModels, 'id');
			
			$allPosModels = JobpositionBase::find()->select(['id'])->where(['status' => 1])->andFilterWhere(['in' , 'companyid' , $allCompID])->all();
			...
			//$allJobposID = array(0);
			$query->andFilterWhere(['in' , 'id' , $allJobposID]);
		}
		...
		if($ort)
		{
			...
			if(count($ortlist) > 0)
			{
				...
				foreach ($ortlist as $idx => $ort)
				{
					...
					{
						$ortqr[] = ['like', 'postcode', $ort];
					}
					else
					{
						$ortqr[] = ['like', 'country', $ort];
						$ortqr[] = ['like', 'city', $ort];
					}
		...
		if($text)
		{
			...
			{
				if(trim($text) == '') unset($textlist[$idx]);
			}
			if(count($textlist) > 0)
			{
				$ortqr = array('or');
				foreach ($textlist as $idx => $text)
				{
					...
					$ortqr[] = ['like', 'title', $text];
					$ortqr[] = ['>', '(select count(*) from j2j_jobpositionskill where jobid=j2j_jobposition.id and skill like "%' . $text . '%")', 0];
	...
	public function searchFavoriteJob($params , $userid, $order = array('createdate' => SORT_DESC))
	{
		...
		$allfav = CandidatefavoriteBase::find()->select(['jobposid'])->where(['userid' => $userid])->all();
		
		$allfav = ArrayHelper::getColumn($allfav, 'jobposid');
		$allfav[] = 0;
		//print_r($allfav); exit;
		
		$query->andFilterWhere(['in', 'id' , $allfav]);
		if (!$this->validate()) {
			// uncomment the following line if you do not want to return any records when validation fails
			// $query->where('0=1');
			return $dataProvider;
	...
	public function searchApplyJob($params , $userid, $order = array('createdate' => SORT_DESC))
	{
		...
		$allfav = CandidatejobapplyBase::find()->select(['jobposid'])->where(['userid' => $userid])->all();
